...
import '../../api/model/stakeNow/StakeNowRequest.dart';
...
class StakeNowByPackageController extends GetxController {
  var userIdError="".obs;
  var typeError="".obs;
  ...
  String stakeType=Get.arguments;
  ...
  Future<void> getUserData( String userId,bool isFromClick) async {
    ...
    await api.getDetails(AppSessionBasicRequest(
        request: jsonDecode('{"strUserID":"$userId","stakeType":"$stakeType","BussinessEventID":2}'),
  ...
  void setWallet(WalletTypeList item){
    ...
    if(item.displayConversionCurrId!=selectedPackage.value.defaultCurrencyId) {
      if (dashboardController.currencyLiveRateMap.isNotEmpty &&
          dashboardController.currencyLiveRateMap.containsKey("${item.displayConversionCurrId ?? 0}-${selectedPackage.value.defaultCurrencyId ?? 0}")) {
...
Future<void> getLiveRate( int fromCurrId,int toCurrId) async {
  ...
  void submitDetail() async {
    ...
    if ((topupDataByUserIdResponse.value.userID ?? 0) == 0) {
      ...
      return;
    }else if ((selectedWallet.value.id ?? 0) == 0) {
      walletError.value = "Please Select Wallet";
      return;
    } else if (convertAmountController.text.trim().isEmpty) {
      convertAmountError.value = "Please Enter Convert Amount";
      return;
    } else if (dashboardController.currencyBalanceMap.isNotEmpty && dashboardController.currencyBalanceMap.containsKey(selectedWallet.value.displayConversionCurrId)) {
      if (double.parse(dashboardController.currencyBalanceMap[selectedWallet.value.displayConversionCurrId ?? 0]!.balance ?? "0") < convertedAmount.value) {
  ...
  Future<void> stakeNow() async {
    DialogBuilder.INSTANCE.showLoadingIndicator('', 'Staking ...', isCancelabel: false);
    await api.stakeNow(AppSessionBasicRequest(
        request: StakeNowRequest(TopupUserId:  topupDataByUserIdResponse.value.userId??userIdController.text.trim(),
            PackageAmount: selectedPackage.value.isRangePackageCost==true?amountController.text.trim():"${selectedPackage.value.packageCost}",
            WalletId: selectedWallet.value.id??0,BusinessType: selectedType.value.oid??0,FromCurrencyId: selectedWallet.value.currencyID??0,PackageID: selectedPackage.value.packageId??0,
            StakeType:topupDataByUserIdResponse.value.stakeType??int.parse(stakeType.isNumericOnly?stakeType:"0")),
        appSession: BasicRequest(loginResponse.data!.loginTypeID, loginResponse.data!.userID, loginResponse.data!.session, loginResponse.data!.sessionID,
            APP_ID, DEVICE_ID, "", packageInfo.version, DEVICE_ID)), (action, response) async {
      DialogBuilder.INSTANCE.hideOpenDialog();
      if (action == TypeActions.INSTANCE.SUCCESS) {
        Utility.INSTANCE.dialogIconOneButton("check", "", response, "Cancel");
        await dashboardController.stakeBalance(true);
        /*if((topupDataByUserIdResponse.value.stakeType??int.parse(stakeType.isNumericOnly?stakeType:"0"))==1){
